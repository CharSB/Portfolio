...
DOWNLOAD_DIR = "ocean_downloads"


def fetch_selections(username, password, recording_id, access_token):
...
def download_species(username, password, species_id, access_token):
...
def download_recording(username, password, encounter_id, access_token):
...
def download_selection_file(username, password, selection_file_id, access_token, species_id):
    """Download a selection file and save it with the given filename."""
    access_token = get_access_token(username, password)
    url = f"{FILES_ENDPOINT}?id={selection_file_id}"
    response = requests.get(url, stream=True,
                            headers=create_authorization_header(access_token))  # Stream for large files
...
def download_data(username, password, format):
    ...
    access_token = get_access_token(u, p)
    for species_id in species_ids:
        count = 0
        # Fetch species from an encounter
        encounter_ids = download_species(u, p, species_id,access_token)

        for encounter_id in encounter_ids:
            # Fetch recording from an encounter
            recording_ids = download_recording(u, p, encounter_id,access_token)
            for recording_id in recording_ids:
                # Fetch selections from recording
                selections = fetch_selections(u, p, recording_id, access_token)
                ...
                for selection in selections:
                    ...
                    if selection_file_id:
                        count += 1
                        if (format == "1"):
                            download_selection_file(u, p, selection_file_id, access_token, species_id)
                        else:
